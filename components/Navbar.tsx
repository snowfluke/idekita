/** The sticky floated navigation bar wrapper that showed app on every single page of the site */
        ...
        <nav className="md:flex justify-between items-center">
          ...
          {isMainpage && (
            <NavSearch
              toggle={
                navSearch
                  ? "relative max-w-lg ml-2 mt-2 mb-1 flex justify-center hidden md:block"
                  : "relative max-w-lg ml-2 mt-2 mb-1 flex justify-center block"
              }
            />
          )}
          {!isMainpage && (
            <NavMiddle
              toggle={
                menuMiddle
                  ? "md:flex text-center mt-3 md:mt-0 mx-2 hidden"
                  : "md:flex text-center mt-3 md:mt-0 mx-2 block"
              }
            />
          )}
          ...
          {username && (
            <NavProfile
              toggle={
                menuProfile
                  ? "relative text-left md:block hidden"
                  : "relative text-left block"
              }
              user={userData}
            />
          )}
          {!username && (
            <ul
              className={
                menuNav
                  ? "items-center md:flex text-center mt-3 md:mt-0 mb-3 md:mb-0 hidden"
                  : "items-center md:flex text-center mt-3 md:mt-0 mb-3 md:mb-0 block"
              }
            >
              <a onClick={signIn}>
